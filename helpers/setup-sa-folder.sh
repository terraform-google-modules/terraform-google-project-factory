...
# - This setup helper version binds IAM to the Folder instead of Org wherever possible
# - This is to meet requirements for clients whose teams are constrained to working in a Folder
#   rather than having access to the whole Org.
# - This version leverages `gcloud alpha` for some actions
...
if [ $# -lt 3 ]; then
  echo
  echo "Usage: $0 <organization name> <folder id> <project id> [<billing account id>]"
  echo
...
FOLDER_ID="$(gcloud alpha resource-manager folders list --organization=$ORG_ID --format="value(ID)" --filter="$2")"
...
SEED_PROJECT="$(gcloud projects list --format="value(projectId)" --filter="$3")"
...
if [ $# -eq 4 ]; then
  ...
  BILLING_ACCOUNT="$(gcloud beta billing accounts list --format="value(ACCOUNT_ID)" --filter="$4")"
...
# Grant roles/resourcemanager.projectCreator to the service account on the folder
...
# Grant roles/compute.xpnAdmin to the service account on the folder
...
# Grant roles/compute.networkAdmin to the service account on the folder
...
# Grant roles/iam.serviceAccountAdmin to the service account on the folder
